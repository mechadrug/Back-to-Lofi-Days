...
//             // 根据瓦片类型判断是否可碰撞
//             mapData[y][x].isCollidable = (mapData[y][x].tileType == 478); // 478为可碰撞瓦片
//         }
//     }
...
// class MovableObject {
// public:
//     MovableObject(float x, float y, const sf::Texture& texture)
...
//     // 检测是否与可碰撞瓦片发生碰撞
//     bool checkCollision(float newX, float newY, const vector<vector<Tile>>& mapData, int tileWidth, int tileHeight,int select) {
...
//     vector<vector<Tile>> mapData = load_map_data(loading, width, height);
...
//     // 加载地图纹理
//     Texture mapTexture;
//     if (!mapTexture.loadFromFile("../resources/images/fckmap.png")) {
...
//     MovableObject girl(50.f, 40.f, texture);  // 设定初始位置
