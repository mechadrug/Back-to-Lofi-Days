...
Map::Map(const string &bgFile){
    ...
    float scaleX = static_cast<float>(windowWidth) / static_cast<float>(texture.getSize().x);
    float scaleY = static_cast<float>(windowHeight) / static_cast<float>(texture.getSize().y);
